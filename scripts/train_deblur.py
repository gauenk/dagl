...
from dagl.configs import set_seed
from dagl import exps as exps_menu
...
from dagl.lightning import DaglLit,MetricsCallback
from dagl.utils.misc import rslice,write_pickle,read_pickle,optional
...
def launch_training(_cfg):
    ...
    print(model_cfg)
    model = DaglLit(model_cfg,flow=cfg.flow,isize=cfg.isize,
...
def main():
    ...
    # -- search info --
    exps = exps_menu.train_patch2vid_blur.load("train")

    # -- group with default --
    cfg = exps_menu.train_patch2vid_blur.load("base")
    cfg.seed = 234
    ...
    cfg.noise_version = "rgb_blur" # fixed.
    cfg.scheduler = "default"
    cfg.skip_mismatch_model_load = "true"
    cfg.rbwd = "true"
    cfg.limit_train_batches = 0.005*1.8
